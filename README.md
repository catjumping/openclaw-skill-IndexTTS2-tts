...
[![ClawHub](https://img.shields.io/badge/ClawHub-indextts2--tts-blue)](https://clawhub.ai)
...
### 方法二：手动安装
1. 下载本仓库代码
2. 将 `IndexTTS2-tts` 文件夹放到你的 OpenClaw `skills/` 目录下
3. 重启 OpenClaw，技能会自动被识别加载

### 方法三：Git 安装（方便更新）
```bash
cd /path/to/your/openclaw/workspace/skills
git clone https://github.com/catjumping/openclaw-skill-IndexTTS2-tts.git IndexTTS2-tts
```

更新技能：
```bash
cd skills/IndexTTS2-tts
git pull
```
...
**示例：**
```powershell
py scripts/indextts2_tts.py upload --file "C:/voices/myvoice.wav" --name "我的声音" --describe "温暖旁白"
```
...
**示例：**
```powershell
py scripts/indextts2_tts.py tts --text "你好，欢迎使用IndexTTS2语音合成！" --audio-id ABuXqMU5ZnPCFBHtv93wmnhqLM --output "output.wav"
```
...
```
用 indextts2-tts 合成这句话："今天天气真好，适合出去走走。" 使用我的声音模型。
```
...
### Q: 提示 "未配置API Key"
A: 请设置环境变量 `INDEXTTS2_API_KEY`，或者调用时添加 `--api-key 你的Key` 参数。
