...
- 🔄 **向后兼容**：兼容旧版 LipVoice 环境变量和配置，平滑升级
...
```
> 注：也兼容旧环境变量 `LIPVOICE_API_KEY`，无需重新配置。
...
### Q: 提示 "未配置API Key"
A: 请设置环境变量 `INDEXTTS2_API_KEY`（兼容旧版 `LIPVOICE_API_KEY`），或者调用时添加 `--api-key 你的Key` 参数。
...
- ✅ 零依赖，使用 Python 标准库
- ✅ 支持环境变量和参数两种 API Key 配置方式，兼容旧版 LipVoice 配置
